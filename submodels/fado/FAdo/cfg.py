...
class CFGrammar(object):
    ...
    def __str__(self):
        ...
        for n in range(len(self.Rules)):
            lhs = self.Rules[n][0]
            rhs = self.Rules[n][1]
            if type(rhs) is not str and len(rhs) > 1:
                rhs = " ".join(rhs)
            s += "{0:s} | {1:s} -> {2:s} \n".format(n, lhs, rhs)
...
class CNF(CFGrammar):
    ...
    def iter_rule(self, lhs, rhs, i):
        if type(rhs) is not str and len(rhs) == 2:
    ...
    def Chomsky(self):
        ...
        for a in self.Terminals:
            for i in range(len(self.Rules)):
                if type(self.Rules[i][1]) is not str and len(self.Rules[i][1]) >= 2 and a in self.Rules[i][1]:
        ...
        n = len(self.Rules)
        for i in range(n):
            if type(self.Rules[i][1]) is not str and len(self.Rules[i][1]) > 2:
